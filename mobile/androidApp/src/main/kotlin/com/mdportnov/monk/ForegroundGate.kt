...
class ForegroundGate(
     ...
     */
    fun onUserPresent() {
        val pkg = pendingAfterUnlock
        pendingAfterUnlock = null
        if (pkg != null && pkg == lastForeground) evaluate(pkg) else reevaluateForeground()
    }
    ...
    /**
     * Re-check the app in front (allowance expiry, focus started from a tile, a restart).
     * Under the keyguard the verdict waits for the unlock, like a window event would.
     */
    ...
    fun evaluate(pkg: String) {
        ...
        when (val decision = effects.decide(pkg)) {
            Decision.Allow -> {
                ...
                effects.schedule(delay + 250) { lastForeground?.let { if (it == pkg) evaluate(it) } }
            }
